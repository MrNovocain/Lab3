...
public class Main {
    ...
    public static void runProgram(Translator translator) {
        while (true) {
            ...
            CountryCodeConverter c1 = new CountryCodeConverter();
            String countryCode = c1.fromCountry(country);
            // TODO CheckStyle: The String "quit" appears 3 times in the file.
            // TODO Checkstyle: String literal expressions should be on the left side of an equals comparison
            if (country.equals(QUIT_COMMAND)) {
                break;
            }
            ...
            String lanCode = converter.fromLanguage(language);
    ...
    private static String promptForCountry(Translator translator) {
        List<String> threeLetterCodes = translator.getCountries();
        List<String> fullName = new ArrayList<>();
        CountryCodeConverter converter = new CountryCodeConverter();

        for (String code : threeLetterCodes) {
            fullName.add(converter.fromCountryCode(code));
        }
    ...
    private static String promptForLanguage(Translator translator, String country) {
        ...
        List<String> availableLanCode = translator.getCountryLanguages(country);
        List<String> availableLanName = new ArrayList<>();
        LanguageCodeConverter converter = new LanguageCodeConverter();
        for (String code : availableLanCode) {
            availableLanName.add(converter.fromLanguageCode(code));
        }
        System.out.println(availableLanName);
